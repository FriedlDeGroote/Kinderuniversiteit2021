...
% Modified by Friedl De Groote for National Biomechanics Day
% Date: 10/11/2018
%
clear all;
clc
...
% Save name
name = '1';
% Set hip actuator force (percentage of nominal)
F_hip = 150;
% Set knee actuator force (percentage of nominal)
F_knee = 100;
% Set ankle actuator force(percentage of nominal)
F_ankle = 100;
% Set maximal contraction velocity (human range 5-20)
v_0_hip = 13;
v_0_knee = 11;
v_0_ankle = 11;
...
% Variable settings
settings = [10, 4, 50, 1, 1, 1, 1, 2; % case 1
    10, 4, 50, 1, 1, 0, 1, 2; % case 2
]; 
...
%% Settings
% Variable
v_tgt       = settings(ww,1);    % weight maximizing velocity
...
savename = ['case',name,...
    '_hip',num2str(round(F_hip)), '_v',num2str(round(v_0_hip)),...
    '_knee',num2str(round(F_knee)), '_v',num2str(round(v_0_knee)), ...
    '_ankle',num2str(round(F_ankle)),'_v',num2str(round(v_0_ankle)),...
    '_kT',num2str(kT)];

%% External function
pathmain = pwd;
% Path to external function
if strcmp(getenv('COMPUTERNAME'),'GBW-L-W2016')
    pathexternal = ['C:\Users\u0046458\Documents\', ...
        'MATLAB\predictivesimulations_rvc_dependencies', ...
        '\PredSim_s1_2D-install\bin'];                  
end
...
if (F_hip + F_knee + F_ankle - 300) > 50
   F_hip = 100
   F_knee = 100
   F_ankle = 100
   pause
...
if 100*(v_0_hip + v_0_knee + v_0_ankle - 30)/10 > 50
   v_0_hip = 10
   v_0_knee = 10
   v_0_ankle = 10
   pause
...
MTparameters_m(1,iHip) = F_hip/100 * MTparameters_m(1,iHip);
MTparameters_m(1,iKnee) = F_knee/100 * MTparameters_m(1,iKnee);
MTparameters_m(1,iAnkle) = F_ankle/100 * MTparameters_m(1,iAnkle);

MTparameters_m(end,iHip) = MTparameters_m(end,iHip)*v_0_hip/10;
MTparameters_m(end,iKnee) = MTparameters_m(end,iKnee)*v_0_knee/10;
MTparameters_m(end,iAnkle) = MTparameters_m(end,iAnkle)*v_0_ankle/10;

aTendon = 35*ones(NMuscle,1);
iPF = [7 8];
aTendon([iPF iPF+NMuscle/2]) = kT;

% MTparameters_m(1,:) = 2*MTparameters_m(1,:);
...
if solveProblem
    ...
    for k=0:N-1
        ...
        [Hilldiffk,FTk] = f_ForceEquilibrium_FtildeState_Tlin(...
                ak,FTtildek.*scaling.FTtilde',...
                dFTtildek.*scaling.dFTtilde,lMTk_lr,vMTk_lr,aTendon);            
...
if reconstructGaitCycle                
    ...
    savenameIK = ['v', num2str(round(vel_aver_opt * 1000)), savename];
